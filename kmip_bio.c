                                               ...
                                               char **id, int *id_size)
{
    ...
    *id = result_id;
    ...
    fprintf(stderr, "kmipcreate#10: is OK\n");
...
int kmip_bio_locate_with_context(KMIP *ctx, BIO *bio, Attribute *attributes, int attribute_count, int maximum_items, int* ids_count, char **uuid, int *uuid_size)
{
    ...
    uint8 *encoding = (uint8*)ctx->calloc_func(
        ctx->state,
        buffer_blocks,
        buffer_block_size);
    if(encoding == NULL)
...
//        id.size = strlen(name);
    ...
    while(encode_result == KMIP_ERROR_BUFFER_FULL)
    {
        ...
        encoding = (uint8*)ctx->calloc_func(
            ctx->state,
            buffer_blocks,
            buffer_block_size);
        if(encoding == NULL)
    ...
    encoding = (uint8*)ctx->calloc_func(ctx->state, buffer_blocks, buffer_block_size);
    if(encoding == NULL)
    {
        return(KMIP_MEMORY_ALLOC_FAILED);
    }
    ...
    kmip_set_buffer(ctx, NULL, 0);
    uint8 *extended = (uint8*)ctx->realloc_func(
        ctx->state,
        encoding,
        buffer_total_size + length);
    if(encoding != extended)
